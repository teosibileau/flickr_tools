...
def addPhoto(access,uset,photo):
    ...
    if not flickr.photos_getPerms(photo_id=photo.get('id'))[0].get('ispublic')==1:
        print flickr.photos_setPerms(photo_id=photo.get('id'),is_public=1,is_friend=0,is_family=0,perm_comment=3,perm_addmeta=0)[0]
    try:
